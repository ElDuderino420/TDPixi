var EasyStarDemo = function(width, height) {
	...
	var iterationsPerCalculation = 8;
	...
	var populateBoardWithObjects = function(board) {
		...
		for (var y = 0; y < board.length; y++) {
			for (var x = 0; x < board[0].length; x++) {
				if (board[y][x] == 1) {
					createBoardSprite(subtextures[Math.floor(Math.random()*5)], objectContainer, x * TILE_WIDTH + TILE_WIDTH*offset, y * TILE_HEIGHT + TILE_HEIGHT*offset);
				} else if (board[y][x] == 2) {
					createBoardSprite(subtextures[5], objectContainer, x * TILE_WIDTH + TILE_WIDTH*offset, y * TILE_HEIGHT + TILE_HEIGHT*offset);
				}
			}
		}
	};
